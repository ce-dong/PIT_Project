...
class FinaIndicatorUpdater(BaseUpdater):
    ...
    def run(self, context: UpdateContext) -> dict[str, object]:
        ...
        for idx, ts_code in enumerate(stock_codes, start=1):
            ...
            frame = self.client.fetch_fina_indicator(ts_code=ts_code, start_date=start_date, end_date=end_date)
            if not frame.empty:
                frames.append(frame)

        df = self._concat_frames(frames)
        if not df.empty:
            df = normalize_date_columns(df, ["ann_date", "end_date"])
